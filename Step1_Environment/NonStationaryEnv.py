...
class NonStationaryEnv(Environment):
    def __init__(self, alphas, weights, returnerWeights, M, M0, theta, prices, costs, pages, maxQuantity,horizon):
        super().__init__(alphas, weights, returnerWeights, M, M0, theta, prices, costs, pages, maxQuantity)
        self.t = 0
        n_phases = len(self.M)
    ...
    def returningLandingProduct(self, user):
        ...
        if user.discountedItem is not Products.P0:
            ...
            prob = [self.M0[current_phase][user.firstLandingItem][p] for p in buyableProducts()]
