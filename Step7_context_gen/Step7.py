...
def returningVisit(user1):
    ...
    if landingProduct < 5 and oldDiscountedItem < 5:
        learnersWeights[oldDiscountedItem].updateEstimates(u.episode)
        episodes_set[oldDiscountedItem].append(copy.deepcopy(u.episode))
    return m
if __name__ == '__main__':
    ...
    for e in range(n_experiment):
        ...
        episodes_set=[[] for _ in range(5)]
        possibleReturnersAtTimeT = []
        instantRegret = []
        context_gen=context_generator(prices-costs,prices)
        learnersWeights = context_gen.init()
        instantReward=[]
        ...
        for t in range(horizon):
            ...
            del userVisitingToday,margin,dailyOptimalMargins,dailyMargins,returnerWeightsEstimated
